        ...
        void OnAttach(TargetPort target)
        {
            ...
            switch (directionType)
            {
                case DirectionType.Horizontal:
                    {
                        //if(itemName != "晶体检波器") transform.position = new Vector3(target.targetPosition.x, target.targetPosition.y, transform.position.z);
                        //else
                        //{
                ...
                case DirectionType.Vertical:
                    {
                        transform.position = new Vector3(target.targetPosition.x-0.05f, target.targetPort.transform.position.y, target.targetPort.transform.position.z);
                    }
                    break;
                default:
                    break;
            ...
            for (int i = 0; i < linkConditions.Count; i++)
            {
                ...
                else
                {
                    ...
                    if (itemName == "晶体检波器")
                    {
                        ...
                        {
                            //RotationY();
                            //Debug.LogError("厚度1："+ _targetPort.portCollider.bounds.size.x);
                            targetPort.distance = _targetPort.portCollider.bounds.size.x;
                        }
                        else
                        {
                            //Debug.LogError("厚度2：" + _targetPort.portCollider.bounds.size.z);
