...
public class ExcelCodeCreater
{
    ...
    private static StringBuilder CreateExcelAssetClass(ExcelMediumData excelMediumData)
    {
        ...
        foreach (var item in propertyNameTypeDic)
        {
            ...
            classSource.Append(AssignmentCodeProperty("allItemValueRowList[i][\"" + item.Key + "\"]", propertyNameTypeDic[item.Key], item.Key, allItemValueRowList));
            classSource.Append(";\n");
        }
        classSource.Append("\t\t}\n");
    ...
    //声明Asset操作类字段
    private static string AssignmentCodeProperty(string stringValue, string type, string keyName, List<Dictionary<string, string>> allItemValueRowList)
    {
        //判断类型
        if (type == "int" || type == "Int" || type == "INT")
        {
