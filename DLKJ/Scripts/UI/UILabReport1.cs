...
namespace DLKJ
{
    public class UILabReport1 : UILabReportBase
    {
        ...
        public override void SaveData()
        {
            ...
            labReport1Data.EquivalentSectionPosition = StringToDouble(EquivalentSectionPosition.text)*0.001f;//等效截面位置
            labReport1Data.InputWavelength = StringToDouble(InputWavelength.text) * 0.001f;//输入端波长
            labReport1Data.VariableShortCircuitFirstPos = StringToDouble(VariableShortCircuitFirstPos.text) * 0.001f;//可变短路器第一波节点位置
            labReport1Data.VariableShortCircuitSecondPos = StringToDouble(VariableShortCircuitSecondPos.text) * 0.001f;//可变短路器第二波节点位置
            labReport1Data.VariableWavelengthInShortCircuit = StringToDouble(VariableWavelengthInShortCircuit.text) * 0.001f;//可变短路器中波长 
            labReport1Data.OpenLoadPosition = StringToDouble(OpenLoadPosition.text) * 0.001f;//开路负载位置
