...
{
    [Header("Base Data")]
    [SerializeField] ModelType modelType = ModelType.Chain;
    [SerializeField] Transform startT;
    [SerializeField] Transform endT;
    [SerializeField] Mesh startMesh;[SerializeField] Material startMaterial;
    [SerializeField] Mesh endMesh;[SerializeField] Material endMaterial;
    ...
    void Update()
    {
        ...
        {
            startLock = cam.ScreenToWorldPoint(new Vector3(Input.mousePosition.x, Input.mousePosition.y-10f, mouseOffset));
        }
        else if (isStartLocked && !isEndLocked)
        {
        ...
        if (isStartLocked && isEndLocked)
        {
            ...
            {
                isStartLocked = false; isEndLocked = false;
            }
        }
        Vector3 originPos = currentNodePositions[0];
        if (startT) startT.position = originPos + new Vector3(0, nodeDistance, 0);
        if (endT) endT.position = currentNodePositions[totalNodes - 1];

        DrawRope();
        
        Graphics.DrawMeshInstanced(link, 0, linkMaterial, matrices, totalNodes);     // Instanced drawing here is really performant over using GameObjects
        //Graphics.DrawMeshInstanced(startMesh, 0, startMaterial, matrices, 1);
        //Graphics.DrawMeshInstanced(endMesh, 0, endMaterial, matrices, 1);
    }
